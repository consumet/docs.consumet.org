...
<Tabs items={["JavaScript", "Python"]}>
  <Tab>
    <>
        ...
        /*
        Using the example episode ID of 'spy-x-family.6ll19/ep-1',
        explicitly defining default server for demostrative purposes.
        */
        const url = "https://api.consumet.org/anime/9anime/watch?episodeId=spy-x-family.6ll19/ep-1&server=vidcloud"
        const data = async () => {
            try {
                const { data } = await axios.get(url, { params: {
                    episodeId: "spy-x-family.6ll19/ep-1",
                    server: "vidcloud"
                } });
                return data;
            } catch (err) {
  ...
  <Tab>
    <>
        ...
        explicitly defining default server for demostrative purposes.
        """
        url = "https://api.consumet.org/anime/9anime/watch?episodeId=spy-x-family.6ll19/ep-1&server=vidcloud"
